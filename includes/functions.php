...
function wpforo_urlencode( $str ) {
	if( ! preg_match( '#^(\#post-\d+|https?:)$#isu', $str ) && ! preg_match( '#([\?\&][^\?\&/=\r\n]*=?[^\?\&/=\r\n]*)(?1)*$#isu', $str ) && strpos( $str, '~' ) === false && strpos( $str, '*' ) === false && $str === urldecode( $str ) ) {
		$str = urlencode( $str );
	}
...
function wpforo_apply_email_shortcodes( $txt, $pitem, $item, $owner, $user, $unsubscribe_link ){
	$body_length = apply_filters( 'wpforo_email_notification_post_body_length', 0 );
    ...
    if( strpos( $txt, '[forum_link]' ) !== false ){
	    $txt = stripslashes( str_replace(
            '[forum_link]',
            sprintf( '<a target="_blank" href="%1$s">%2$s</a>', esc_url( wpforo_forum( $forumid, 'url' ) ), sanitize_text_field( wpforo_forum( $forumid, 'title' ) ) ),
            $txt
        ) );
    }
	if( strpos( $txt, '[topic_link]' ) !== false ){
		$txt = stripslashes( str_replace(
             '[topic_link]',
             sprintf( '<a target="_blank" href="%1$s">%2$s</a>', esc_url( wpforo_topic( $topicid, 'url' ) ), sanitize_text_field( wpforo_topic( $topicid, 'title' ) ) ),
             $txt
        ) );
	}
	if( strpos( $txt, '[post_link]' ) !== false ){
		$txt = stripslashes( str_replace(
             '[post_link]',
             sprintf( '<a target="_blank" href="%1$s">%2$s</a>', esc_url( wpforo_post( $postid, 'url' ) ), sanitize_text_field( wpforo_post( $postid, 'title' ) ) ),
             $txt
        ) );
	}
    if( strpos( $txt, '[unsubscribe_link]' ) !== false ){
		$txt = stripslashes( str_replace(
             '[unsubscribe_link]',
             $unsubscribe_link ? sprintf( '<br><a target="_blank" href="%1$s">%2$s</a>', esc_url( $unsubscribe_link ), wpforo_phrase( 'Unsubscribe', false ) ) : '',
             $txt
        ) );
	}

	$txt = preg_replace_callback( '#\[user_([^\[\]]+?)]#iu', function( $match ) use ( $user ) {
		$value = wpforo_user_field_shortcode_to_value( '['. $match[1] .']', $user['userid'] );
		if( ! $value || ! ( is_string( $value ) || is_numeric( $value ) ) ) $value = '';
		return $value;
	}, $txt );

	$txt = preg_replace_callback( '#\[owner_([^\[\]]+?)]#iu', function( $match ) use ( $owner ) {
		$value = wpforo_user_field_shortcode_to_value( '['. $match[1] .']', $owner['userid'] );
		if( ! $value || ! ( is_string( $value ) || is_numeric( $value ) ) ) $value = '';
		return $value;
	}, $txt );

	$txt = preg_replace_callback( '#\[forum_([^\[\]]+?)]#iu', function( $match ) use ( $forumid, $pitem, $item, $body_length ) {
	...
	$txt = preg_replace_callback( '#\[topic_([^\[\]]+?)]#iu', function( $match ) use ( $topicid, $pitem, $item, $body_length ) {
	...
	return preg_replace_callback( '#\[post_([^\[\]]+?)]#iu', function( $match ) use ( $postid, $item, $body_length ) {
