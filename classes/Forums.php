...
class Forums {
	...
	public static function _get_forumids( $table, $layout = null ) {
        ...
        if( !is_null( $layout ) ) $sql .= WPF()->db->prepare( " AND `cat_layout` = %d", $layout );
	...
	public function merge( $forumid = 0, $mergeid = 0 ) {
		...
		if( $child_forumids = $this->get_child_forums( $forumid ) ) {
			...
			if( ! WPF()->db->query( "UPDATE " . WPF()->tables->forums . " SET `parentid` = " . $mergeid . ", `cat_layout` = " . $merge_layout . " WHERE `forumid` IN(" . $forumids . ")" ) ) {
            ...
            if( $childs = $this->get_childs( $forumid ) ){
	            WPF()->db->query( "UPDATE " . WPF()->tables->forums . " SET `cat_layout` = " . $merge_layout . " WHERE `forumid` IN(" . implode( ',', $childs ) . ")" );
            }
		}
