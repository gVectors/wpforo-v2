...
class SEO {
	...
	public function __construct() {
		$stylesheet_url   = preg_replace( '/(^http[s]?:)/', '', WPFORO_URL . '/assets/css/wpforo-sitemap.xsl' );
		...
		$this->init();
		add_action( 'wpforo_before_init', [ $this, 'before_init' ] );
	}
	...
	public function before_init() {
		if( ! wpforo_setting( 'seo', 'seo_profile' ) ) WPF()->settings->seo['members_sitemap'] = false;
	}
	...
	public function get_sitemap( $type, $paged ) {
		...
		if( ! $xml = get_transient( $transient_key ) ) {
			if( $type ) {
				...
				switch( $type ) {
					case 'topic':
						if( wpforo_setting( 'seo', 'topics_sitemap' ) ) $urls = $this->topics_sitemap( $paged );
					break;
					case 'forum':
						if( wpforo_setting( 'seo', 'forums_sitemap' ) ) $urls = $this->forums_sitemap();
					break;
					case 'profile':
	...
	public function clear_cache( $type = '', $paged = 1 ) {
		...
		switch( $type ) {
			...
			break;
			case 'forum':
				if( ! wpforo_setting( 'seo', 'forums_sitemap' ) ) return;
			break;
			case 'topic':
				if( ! wpforo_setting( 'seo', 'topics_sitemap' ) ) return;
			break;
			case '':
			break;
			default:
